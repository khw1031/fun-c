...
type PossiblyHasPromise<T extends any[]> =
  Head<T> extends never
    ? false
...
type _ReturnPipeType<T extends any[], R = Awaited<PipeLast<T>>> =
...
type ReturnPipeType<T extends any[], R = Awaited<PipeLast<T>>> =
  _ReturnPipeType<T, R> extends Promise<never> ? never : _ReturnPipeType<T, R>;

export type { ReturnPipeType };
